...
1. `Sources/Corpus`(라이브러리) 또는 `Sources/CorpusApp`(엔트리포인트가 필요한 것)에 재현 코드를 넣습니다. 테스트 전용 케이스는 `Tests/CorpusTests`, Objective-C 는 `Sources/CorpusObjC`(실제 Clang 인덱스의 선언·참조와 `bridges`의 컴파일러 식별자, 관찰 한계를 검증합니다).
