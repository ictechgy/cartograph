...
```bash
mint install ictechgy/cartograph@0.6.0
```
...
```swift
// Package.swift
.package(url: "https://github.com/ictechgy/cartograph", revision: "0.6.0"),
```
...
$ cartograph bridges
{
  ...
  "project" : "/app/ios",
  "target" : "flutter",
  "tool" : { "name" : "cartograph", "version" : "0.6.0" },
  "version" : 1
}
```
...
- **도달 불가한 타입 안의 보존된 멤버는 여전히 "보존됨"이라고 답합니다.** `View`를 만족시키는
  `body`나 `Codable`을 만족시키는 `encode(to:)`는 그 타입을 아무도 만들지 않아도 보존 규칙이
  살립니다. `dead`는 멤버가 아니라 타입을 보고하므로 일괄 정리는 옳지만, 멤버만 `query`로
  물으면 프레임워크가 호출한다는 답을 받습니다. 그것은 타입이 살아 있을 때만 참입니다.
  결정하기 전에 타입을 먼저 물어보세요.
