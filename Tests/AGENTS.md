...
- 관계 없는 참조를 넓게 귀속시켜 매크로 확장이 앞 선언에 붙던 문제 → 소유자 종류를 제한하는지
- 제외 글롭이 프로젝트 루트의 조상 디렉터리에 걸리던 문제 → `DerivedData` 아래 프로젝트가 살아남는지
- 심볼릭 링크로 지정한 루트에서 순회가 조용히 비던 문제 → 링크로 가리켜도 파일을 찾는지
- 빈 인덱스가 `--strict` 를 통과하던 문제 → 심볼 0개면 종료 코드 2 인지, 탈출구가 그것을 푸는지
