...
## 0.7.0 이후 — 감사 다음 순위 셋 (2026-09-06 저녁)
...
1. **`Candidate` 에 kind·module·location 을 싣는다** [F12]. 이름이 모호할 때 후보 셋이
   전부 같은 문자열로 출력된다. **kartograph · dartograph 와 스키마를 맞춰야 하므로
   세 저장소 동시 변경이다.** 그 조율이 이 항목의 실제 비용이다.
2. **`query --batch`** [F37]. 43건 스윕이 43개 프로세스, 약 43초다. dartograph 가 이미
   `--batch` 와 `symbol-query-batch` v1 을 출하했으므로 그 스키마를 따른다.
...
## Verification (마지막으로 통과한 것, PR #44 시점)

`Scripts/coverage.sh` 93.30%(테스트 628개) · `Scripts/verify-fixtures.sh`(진짜 인덱스) ·
