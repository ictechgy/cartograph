...
## 2026-09-08 — 0.9.0 배포 준비와 함수 간 분석 점검

브리지 #65와 상수/스코프 #66은 각각 `da30d55`, `85f7942`로 main에 병합됐다.
0.9.0은 이 변경과 함수 간 분석에서 발견한 한계 설명 정정을 포함한다.
isthmus #25·#26도 병합됐고 0.2.0 배포 파일을 검증했다. npm 발행은 사용자 2단계 인증이
필요하다. 소비자 발행 확인 전에는 새 Cartograph 태그를 발행하지 않는다.
[함수 간 실측](docs/scans/2026-09-interprocedural-flow.md): 심볼 호출·참조 도달성은 확인했지만
인자/반환/callback/async/inout의 값 전파는 미구현이다. CodeQL·Semgrep CE/Pro·Infer 자료와
재현 하네스에 근거를 남겼다. 이 점검을 새 데이터 흐름 엔진 구현으로 읽지 않는다.
0.9.0 후보는 728 tests, coverage 93.64%, CLI·코퍼스·자기 분석 필수 게이트를 통과했다.
...
_마지막 갱신: 2026-09-08 (PR #62 머지, 0.8.2 릴리스 메타데이터 갱신)._
...
**릴리스**: 0.1.0 → … → 0.5.5 (2026-09-03~05) → 0.6.0 → 0.7.0 (2026-09-06) → 0.8.0 (2026-09-07) → **0.8.1** (2026-09-07 심야, #52·#54·#57 조용한-플래그 거부 세 묶음). 전부 GitHub Release + Homebrew tap(`ictechgy/tap`, `HOMEBREW_TAP_TOKEN` 이 없어 손 갱신) + `brew upgrade` 확인. 이후 #60·#62가 머지됐고 **0.8.2** 릴리스 메타데이터를 준비했다(위 2026-09-08 절).
