...
1. **isthmus 에 계약 피드백을 전달할 것** — 구현하면서 드러난 계약의 빈 곳. `GRAPH-EXCHANGE.md` 를 고치는 쪽은 isthmus 다
   ...
   - 클로저 안의 `case "…"` 는 미결대로 감싸는 선언의 USR 로 귀속했다. `FlutterPlugin` 스타일(`handle(_:result:)` 메서드)에서는 파일에 채널이 하나일 때만 채널을 붙이고 아니면 `null` 이다. isthmus 가 `null` 채널을 어떻게 셀지 정해야 한다
   - `external-retentions` 의 `symbol.qualifiedName` 은 이 도구의 `GraphNode.qualifiedName`(`Module.name`, 타입 없이)과 같은 표기여야 USR 없는 근거가 맞는다. 계약의 예시(`CameraPlugin.takePhoto`)는 타입을 포함해 표기가 다르다
