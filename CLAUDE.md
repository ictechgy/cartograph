...
`cartograph dead` 출력은 진단 줄 + 빈 줄 + 요약 줄 구조이고, 알릴 것이 있으면 요약 줄 뒤에
`limitations:` 블록이 더 붙습니다. 요약 줄이 마지막 줄이 아닙니다.
`tail -n`으로 자르면 앞쪽 진단이 잘려 문제가 사라진 것처럼 보이고, 뒤쪽을 자르면 이 도구가
무엇을 보지 못했는지가 사라집니다. 실제로 앞쪽 착각 때문에 한 번 잘못 판단한 적이 있습니다.
요약 줄의 개수와 한계 블록을 함께 보세요.
