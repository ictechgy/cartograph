...
the script; short ones in the table) and scanned with `cartograph bridges --format json --exclude
'**/example/**'` built from cartograph commit `37411f8` (the scanner changes described below,
unreleased as of 0.5.3). All 14 scans exited 0. No Flutter SDK was involved: `bridges` walks the
...
both the file counts and the facts. "Pigeon Swift files" counts files whose generator header reads
`Autogenerated from Pigeon`; the `BasicMessageChannel` column, counted by the scanner, is the more
reliable measure of Pigeon use because generated files from older Pigeon versions carry other
headers.
...
The four handler columns are disjoint. "attributed": the channel came from something the code
states — a registration call (`addMethodCallDelegate`, `setMethodCallHandler` with a closure or a
method reference) for Flutter, or the `RCT_EXPORT_*` / `@objc(Name)` declaration for React Native.
"inferred": the fallback where a file constructs exactly one channel and a `FlutterMethodCall`
handler is attributed to it; the registration lives in another file. "channel null": a handler the
scanner saw but could attach to no channel. "dynamic" counts facts of any kind whose channel or
...
files carry the current Pigeon header, so the rest are older generated code, but they are generated
code all the same. flutterfire is mid-migration (150 message channels, 4 method channels left). For these, the join isthmus performs today has nothing to join; the exchange format needs a
